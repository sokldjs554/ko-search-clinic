...
FAILING: list[EvalQuery] = [
    ...
    _q("백팩", {"P110": 2},
       "garbage_split", ("user_word", "synonym"),
       "백팩→[백,팩]. 0건이 아니라 에코백·물티슈 등 쓰레기 4건이 잡히고, "
       "표준형 '배낭'은 자모가 닮지 않아 형태로 유추 불가 — 가장 어려운 케이스"),
]

# 기준선에서 결과가 0건인 질의. 나머지 실패 질의는 '결과는 나오는데 틀린'
# 형태(순위 실패·재현율 실패·쓰레기 유입)라 0건 검사를 적용하면 안 된다.
ZERO_RESULT_AT_BASELINE = (
    "후라이팬", "케잌 주문", "도너츠", "쥬스", "소세지", "카페트",
    "초콜렛", "리모콘", "핸드폰", "츄리닝", "티슈", "머스캣", "아답터",
)
