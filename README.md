...
고치는 것**의 경계가 정확히 측정됩니다:

| 계열 | 예 | 휴리스틱 | 왜 |
|---|---|---|---|
| 표기 변형 (8건) | 후라이팬→프라이팬, 케잌→케이크 | ✅ 전부 치유 | 자모 편집 거리가 잡는다 |
| 복합어 통짜 (2건) | 티슈↛물티슈, 머스캣↛샤인머스캣 | ✅ 전부 치유 | 부분어 포함 규칙이 잡는다 |
| 영한 혼용 (2건) | 블루투스↛Bluetooth, 텀블러↛tumbler | ❌ | 자모 유사도는 문자 체계를 못 넘는다 |
| 유의어 (2건) | 츄리닝↛트레이닝복, 핸드폰↛휴대폰 | ❌ | 형태가 다른 같은 뜻 — 의미 이해 필요 |
| 쓰레기 분해 (1건) | 아답터→[아,답,터] | ❌ 오진 | 사전 등록→동의어의 **2단계 처방**을 모른다 |

남은 5건이 곧 LLM 의사(`--engine claude`)의 몫이며, 같은 게이트를 통과해야
채택되는 것은 동일합니다. **LLM의 가치가 "그럴듯함"이 아니라 치유율 격차로
측정됩니다.**
