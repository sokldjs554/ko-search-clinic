...
## 핵심 결과 — 같은 게이트, 다섯 의사, 전 16건 실측

의사 다섯이 **동일한 평가셋 16건**을 순회 진료했습니다. 같은 상태 기계에 자를
하나씩 더 준 구성이라, 다섯의 차이가 곧 **"그 자가 무엇을 더 푸는가"**입니다.
채택 여부는 전부 같은 회귀 게이트가 판정했습니다.
...
가운데 두 열(`rules+`, `vector+`)은 **비교가 공정했는지 되물어 만든 팔**입니다.
"LLM이 이긴 것은 추론이 아니라 액션 스페이스(`user_words`)와 재시도 아니냐"는
반론을 받고, 규칙 의사에 그 둘을 붙여 다시 쟀습니다. 회수된 것은 **1건**
(`아답터`)이고, 임베딩까지 다 켜도 **12/16**에서 멈춥니다 —
[측정 전문](docs/FAIRNESS.md). LLM을 호출하지 않으므로 API 키 없이 재현됩니다:
...
| 계열 | 예 | 자모 | +임베딩 | 규칙 최대치 | Claude | 경계선의 이유 |
|---|---|---|---|---|---|---|
| 표기 변형 (8건) | 후라이팬→프라이팬 | ✅ 8/8 | ✅ 8/8 | ✅ 8/8 | ✅ 8/8 | 자모 편집 거리가 잡는다 |
| 복합어 통짜 (2건) | 티슈↛물티슈 | ✅ 2/2 | ✅ 2/2 | ✅ 2/2 | ✅ 2/2 | 부분어 포함 규칙이 잡는다 |
| **영한 혼용 (2건)** | 블루투스↛Bluetooth | ❌ 0/2 | ❌ **0/2** | ❌ **0/2** | **✅ 2/2** | 임베딩도 액션도 못 넘는다 — 아래 참조 |
| **유의어 (2건)** | 츄리닝↛트레이닝복, 핸드폰↛휴대폰 | ❌ 0/2 | ⭕ **1/2** | ⭕ **1/2** | **✅ 2/2** | 임베딩이 딱 하나 메운다 |
| **쓰레기 분해 (2건)** | 아답터→[아,답,터] | ❌ 0/2 | ❌ **0/2** | ⭕ **1/2** | **✅ 2/2** | 사전 등록→동의어의 **2단계 처방**. 액션을 주면 `아답터`는 풀리고 `백팩`은 안 풀린다 — 자모로 이어지는 색인 토큰이 없다 |
...
이 프로젝트의 결론은 전부 **문서 72건 · 질의 36건** 위에서 나왔습니다. 그래서
합성 코퍼스로 규모를 키워 같은 처방을 다시 던져봤습니다. 특히 의심스러운
지점이 있었습니다 — BM25 점수는 IDF 항에 선형으로 비례하고 그 IDF는
`log(1 + (N-df+0.5)/(df+0.5))`로 **문서 빈도의 로그**이므로, **같은 패치라도
규모에 따라 안전성이 달라질 수 있습니다.**
...
늘었습니다 — 처방마다 샌드박스를 통째로 재색인하기 때문입니다. 이 기울기면
문서 100만 건에서 처방 하나에 **약 9분**, 진료 16건이면 하루가 갑니다.
**벽은 정확성이 아니라 비용**이고, 실서비스에 올리려면 증분 재색인이
먼저입니다. 전수 재색인은 "빠뜨린 문서가 없다"는 것이 자명해서 고른 방식이고,
그 단순함의 값을 여기서 처음 수치로 확인했습니다.

### 무엇을 고칠지는 로그가 고릅니다

지금까지 진료 대상은 제가 손으로 고른 16건이었습니다. 실서비스에서는
**로그가 고릅니다.** 질의 로그 50만 줄을 Parquet 데이터 레이크(날짜 파티션)로
두고 제로결과 질의를 집계했습니다.
...
우선순위입니다. 제로율이 100%여도 아무도 안 치는 질의는 나중입니다.

**Spark와 DuckDB가 같은 SQL로 같은 결과를 냅니다.** 다만 이 규모에서는
DuckDB가 **48배 빨랐습니다**(0.27초 vs 13초) — JVM 기동과 셔플 비용 때문입니다.
Spark가 값을 하는 것은 로그가 한 대 메모리를 넘을 때이고, **규모가 아닌데
Spark를 쓰는 것은 비용만 냅니다.** 둘 다 두고 결과가 같은지 테스트가 확인합니다.

### 진료가 프로세스보다 오래 삽니다

세션·패치 원장을 PostgreSQL에 둡니다(`DATABASE_URL`). 저장하는 것은
**처방과 그 증거뿐**입니다 — 색인이나 분석기 상태는 처방으로부터 결정적으로
재구성되므로 저장하면 두 벌이 되고, 두 벌은 갈라집니다. 처음부터 처방을
**구조화된 IR**로 받은 값이 여기서 나옵니다.

만들면서 결함을 하나 잡았습니다. 재시작 후 설정은 살아나는데 **원장이
비어 있었습니다** — "왜 이 사전 항목이 있는지"가 사라진 겁니다. 운영 중
재시작 뒤가 그 근거가 가장 필요한 순간이라 고쳤고, 회귀 테스트로 고정했습니다.

DB가 없으면 메모리로 돕니다. **DB 없이 도는 것이 기본 상태**이고, PostgreSQL은
여러 대로 띄울 때 필요해지는 선택지입니다.
...
| 영속 저장소 | 세션·원장을 PostgreSQL에. 저장하는 것은 처방과 증거뿐 — 설정은 처방으로부터 재구성된다 |
...
# 10) RAG — 도메인 지식을 프롬프트가 아니라 검색으로
pip install -e ".[scale]"
clinic rag-eval --compare                     # 검색만 채점 (모델·API 키 불필요)
clinic heal --engine claude --prompt minimal --rag   # 지식을 검색해서 붙인다

# 11) 규모 — 데이터 레이크, 로그 마이닝, 규모 실험
clinic build-lake --docs 50000 --log-rows 500000
clinic mine-logs --engine duckdb               # --engine spark 도 같은 답을 낸다
clinic scale-bench --sizes 0 1000 10000 50000

# 12) 영속 저장소 (선택) — 진료가 프로세스보다 오래 산다
export DATABASE_URL=postgresql://clinic:clinic@localhost:5432/clinicdb

# 테스트 (308개, 전부 오프라인 — API 키도 Docker도 없이 CI가 돈다)
pytest
```
...
├── rag/           # 지식베이스 · 검색(BM25/임베딩/RRF) · 검색 품질 채점
...
tests/             # 308개 테스트 (전부 오프라인 — API 키도 Docker도 불필요)
...
- [docs/FAIRNESS.md](docs/FAIRNESS.md) — **비교가 공정했는가**: 규칙 의사에 액션과 재시도를 주고 다시 잰 기록
