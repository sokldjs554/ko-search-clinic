...
## 실측이 잡아낸 것 — 렌더러의 nori 규약 위반

첫 실행에서 ES가 **인덱스 생성 자체를 거부**했다:

```
failed to build synonyms from ['clinic_synonyms' analyzer settings]
term: 후라이팬 analyzed to a token (후라이) with position increment != 1 (got: 0)
```

`decompound_mode: mixed`는 `후라이팬`을 `[후라이팬, 후라이, 팬]`으로 **겹쳐서**
내보내는데, 동의어 필터는 규칙 안의 표현이 겹치는 토큰으로 분석되면 그 규칙을
만들 수 없다. Kiwi는 `후라이팬`을 한 토큰으로 두므로 **로컬과 nori의 동작
차이**이기도 하다.

렌더러는 "로컬 패치가 nori 설정과 1:1 대응한다"고 주장해 왔지만, 그 주장은
**한 번도 검증된 적이 없었다.** 붙여보니 틀렸다.

**조치.** 동의어에 등장하는 한글 표현을 `user_dictionary_rules`에 원자 토큰으로
등록한다. 규칙이 유효해질 뿐 아니라 **nori를 Kiwi와 같게 맞추는** 조치다.
분해가 의도인 말(`물티슈 물 티슈`)은 덮어쓰지 않고, 라틴 문자는 nori가 분해하지
않으므로 등록하지 않는다.

**그리고 ES 없이도 잡히게 했다.** `validate_es_settings()`가 인덱스 생성 전에
같은 위반을 정적으로 검사하고, 걸리면 ES의 400보다 원인이 분명한 메시지로
막는다. Docker가 있어야만 도는 검사는 CI에서 영영 안 돌기 때문이다
(`test_es_verify.py`의 6건이 이 계약을 고정한다).
