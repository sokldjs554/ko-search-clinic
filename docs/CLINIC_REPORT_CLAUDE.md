...
| 블루투스 스피커 | cross_script | cross_script | synonym | ✅ | 0.50 → 1.00 | 1 |
| 텀블러 | cross_script | cross_script | synonym | ✅ | 0.61 → 1.00 | 1 |
| 핸드폰 | synonym_gap | synonym_gap | synonym | ✅ | 0.00 → 0.67 | 1 |
| 츄리닝 | synonym_gap | synonym_gap | synonym | ✅ | 0.00 → 1.00 | 1 |
...
| 아답터 | garbage_split | garbage_split | user_word+synonym | ✅ | 0.00 → 1.00 | 1 |
| 백팩 | garbage_split | garbage_split | user_word+synonym | ✅ | 0.00 → 1.00 | 1 |

- 치유율: **100%** (16/16)
- 진단 정확도: **100%** — 계열 라벨과 일치 (고쳤어도 이유가 틀리면 오진)
- 처방 적합성: **100%** — 정답 처방 유형을 전부 포함
- 실패셋 평균 nDCG: **0.070 → 0.980**
- 실패셋 제로결과율: **81% → 0%**
...
| 9 | 블루투스 스피커 | cross_script | 동의어(bluetooth = 블루투스) | 0.50 → 1.00 | 3 → 3 | 1 |
| 10 | 텀블러 | cross_script | 동의어(텀블러 = tumbler) | 0.61 → 1.00 | 1 → 2 | 1 |
| 11 | 핸드폰 | synonym_gap | 동의어(휴대폰 = 핸드폰) | 0.00 → 0.67 | 0 → 2 | 1 |
| 12 | 츄리닝 | synonym_gap | 동의어(트레이닝복 = 츄리닝) | 0.00 → 1.00 | 0 → 2 | 1 |
| 13 | 티슈 | compound_locked | 분해확장(물티슈 → 물+티슈) | 0.00 → 1.00 | 0 → 2 | 1 |
| 14 | 머스캣 | compound_locked | 분해확장(샤인머스캣 → 샤인+머스캣) | 0.00 → 1.00 | 0 → 2 | 1 |
| 15 | 아답터 | garbage_split | 사전등록(아답터) + 동의어(어댑터 = 아답터) | 0.00 → 1.00 | 0 → 2 | 1 |
| 16 | 백팩 | garbage_split | 사전등록(백팩) + 동의어(배낭 = 백팩) | 0.00 → 1.00 | 4 → 1 | 1 |
