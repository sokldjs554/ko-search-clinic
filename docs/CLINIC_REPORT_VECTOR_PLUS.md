...
| 아답터 | garbage_split | garbage_split | user_word+synonym | ✅ | 0.00 → 1.00 | 2 |
| 백팩 | garbage_split | synonym_gap ⚠️오진 | — | ❌ | 0.00 → 0.00 | 1 |

- 치유율: **75%** (12/16)
- 진단 정확도: **81%** — 계열 라벨과 일치 (고쳤어도 이유가 틀리면 오진)
- 처방 적합성: **75%** — 정답 처방 유형을 전부 포함
- 실패셋 평균 nDCG: **0.070 → 0.799**
- 실패셋 제로결과율: **81% → 6%**
...
| 9 | 핸드폰 | synonym_gap | 동의어(휴대폰 = 핸드폰) | 0.00 → 0.67 | 0 → 2 | 1 |
| 10 | 티슈 | compound_locked | 분해확장(물티슈 → 물+티슈) | 0.00 → 1.00 | 0 → 2 | 1 |
| 11 | 머스캣 | compound_locked | 분해확장(샤인머스캣 → 샤인+머스캣) | 0.00 → 1.00 | 0 → 2 | 1 |
| 12 | 아답터 | garbage_split | 사전등록(아답터) + 동의어(어댑터 = 아답터) | 0.00 → 1.00 | 0 → 2 | 2 |
